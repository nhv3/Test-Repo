...
namespace BLEHeartRateCollector
{
    ...
    public partial class Form1 : Form
    {
        ...
        //Tells us when the SAMB11 has gotten the byte packge for a prepare write event. This is only toggled when the prepare_write flag is held HIGH. This means that the prepare event has executed fine and now we are waiting for the SAMB11 to acknowledge the event.
        public int remote_acknowledged = 0;
        ...
        public void ATTClientGroupFoundEvent(object sender, Bluegiga.BLE.Events.ATTClient.GroupFoundEventArgs e)
        {
            ...
            {
                ThreadSafeDelegate(delegate { txtLog.AppendText(String.Format("Found attribute group for custom programming service: start={0}, end=%d", e.start, e.end) + Environment.NewLine); });
        ...
        public void ATTClientFindInformationFoundEvent(object sender, Bluegiga.BLE.Events.ATTClient.FindInformationFoundEventArgs e)
        {
            ...
            if (e.uuid.SequenceEqual(new Byte[] { 0xf7, 0xed, 0xf0, 0x2b, 0xcb, 0xed, 0x4b, 0x15, 0x88, 0xe6, 0x75, 0x75, 0xed, 0xab, 0x98, 0x80 }))
            {
                ThreadSafeDelegate(delegate { txtLog.AppendText(String.Format("Found attribute under the programming service. Switching to Read. : handle={0}", e.chrhandle) + Environment.NewLine); });
        ...
        public void ATTClientProcedureCompletedEvent(object sender, Bluegiga.BLE.Events.ATTClient.ProcedureCompletedEventArgs e)
        {
            ...
            {
                route_counter = 0;
                Byte[] cmd = bglib.BLECommandATTClientExecuteWrite(0, 1);
                bglib.SendCommand(serialAPI, cmd);
                while (bglib.IsBusy()) ;
                // DEBUG: display bytes written
        ...
        public void BLEResponseATTClientExecuteWrite(object sender, Bluegiga.BLE.Responses.ATTClient.ExecuteWriteEventArgs e)
        {
            if (e.result == 0)
            {
                ...
                ThreadSafeDelegate(delegate { txtLog.AppendText("BYE!" + Environment.NewLine); });
            }
        ...
        public void ATTClientAttributeValueEvent(object sender, Bluegiga.BLE.Events.ATTClient.AttributeValueEventArgs e)
        {
            ...
            ThreadSafeDelegate(delegate { txtLog.AppendText(String.Format("GATT SERVER: {0} ", ByteArrayToHexString(e.value)) + Environment.NewLine); });
        ...
        private void btnGo_Click(object sender, EventArgs e)
        {
            ...
            ThreadSafeDelegate(delegate { txtLog.AppendText(String.Format("I have entered into Prepare section") + Environment.NewLine); });

            byte[] array = HexStringToBytes(user_input);
            byte[] newArray = new byte[16];
            Array.Copy(array, 0, newArray, 0, 16);
            //Set the initial offset to be 0
            int selector = 0;
            UInt16 T = Convert.ToUInt16(selector);
            remote_acknowledged = 1;
            ThreadSafeDelegate(delegate { txtLog.AppendText(String.Format("I have entered into Prepare section") + Environment.NewLine); });
            //Loop for 7 runs in the queque. Log all of the data that needs to be sent to SAMB11
            for (int i = 0; i <= 7; i++)
            {
                while (remote_acknowledged == 0)
                {
                ...
                ThreadSafeDelegate(delegate { txtLog.AppendText(String.Format("Loop") + Environment.NewLine); });
                ...
                if (i != 7)
                {
                    Array.Copy(array, selector, newArray, 0, 16);
                }
            }
